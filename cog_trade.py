...
class TradeCog(commands.Cog):
    ...
    async def _account(self, db, interaction, now):
        """Fetch the caller's account and enforce the trade cooldown.

        Returns the account row, or None after having already replied with an
        ephemeral error (no account / still on cooldown).
        """
        ...
        if acc["last_trade_at"]:
            ...
            if elapsed < TRADE_COOLDOWN_SECONDS:
                m, s = divmod(int(TRADE_COOLDOWN_SECONDS - elapsed), 60)
                await interaction.response.send_message(
    ...
    async def _open_market(self, db, interaction, market_id):
        async with db.execute(
    ...
    @app_commands.command(name="buy", description="Buy YES or NO shares on a market.")
    ...
    async def buy(
        ...
        async with connect() as db:
            acc = await self._account(db, interaction, now)
            if acc is None:
                return
            if acc["balance"] < amount:
                ...
                return
            m = await self._open_market(db, interaction, market_id)
            if m is None:
                return
    ...
    @app_commands.command(name="sell", description="Sell shares you hold back to the market.")
    @app_commands.describe(
        market_id="ID of the market (see /markets)",
    ...
    async def sell(
        ...
        async with connect() as db:
            ...
            if held <= 0:
                await interaction.response.send_message(
                    f"You don't hold any {outcome.upper()} shares on `#{market_id}`.",
                    ephemeral=True,
                )
                return
            ...
            if sell_shares > held + 1e-9:
                await interaction.response.send_message(
                    f"You only hold **{held:.2f}** {outcome.upper()} shares.", ephemeral=True
                )
                return
            ...
            if payout <= 0:
                await interaction.response.send_message(
                    "Those shares aren't worth anything at the current price.", ephemeral=True
                )
                return
            ...
            await db.execute(
                ...
                (sell_shares if outcome == "yes" else 0.0,
                 sell_shares if outcome == "no" else 0.0,
                 gid, uid, market_id),
            )
            p_yes_after, p_no_after = prices(new_y, new_n, b)
            await db.execute(
        ...
        await interaction.response.send_message(
            f"{interaction.user.mention} sold **{sell_shares:.2f}** {outcome.upper()} shares on "
